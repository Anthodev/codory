...
func TestInstallZshPluginAutosuggestions_CommandStructure(t *testing.T) {
	...
	for platform, cmd := range action.PlatformCommands {
		...
		if !utils.Contains(cmd.Command, "https://github.com/zsh-users/zsh-autosuggestions") {
		...
		if !utils.Contains(cmd.Command, "${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-autosuggestions") {
