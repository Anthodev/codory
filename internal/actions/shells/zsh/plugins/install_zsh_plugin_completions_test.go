...
func TestInstallZshPluginCompletions(t *testing.T) {
	action := InstallZshPluginCompletions()

	if action == nil {
		t.Fatal("InstallZshPluginCompletions() returned nil")
	}

	if action.ID != "install_zsh_plugin_completions" {
		t.Errorf("Expected action ID to be 'install_zsh_plugin_completions', got '%s'", action.ID)
	}

	if action.Name != "Install Zsh Plugin zsh-completions" {
		t.Errorf("Expected action name to be 'Install Zsh Plugin zsh-completions', got '%s'", action.Name)
	}

	if action.Description != "Installs the zsh-completions plugin for oh-my-zsh" {
		t.Errorf("Expected action description to be 'Installs the zsh-completions plugin for oh-my-zsh', got '%s'", action.Description)
	}
...
// TestInstallZshPluginCompletions_PlatformCommands tests that platform commands are correctly configured
// This test only verifies the command strings without executing them
func TestInstallZshPluginCompletions_PlatformCommands(t *testing.T) {
	action := InstallZshPluginCompletions()
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedCommand: "git clone https://github.com/zsh-users/zsh-completions ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-completions",
			expectedSource:  actions.PackageSourceAny,
			expectedCheck:   "which git && (test -d $HOME/.oh-my-zsh || which omz) && test -d ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-completions",
		},
		{
			name:            "macOS platform",
...
// TestInstallZshPluginCompletions_UnsupportedPlatforms verifies that unsupported platforms don't have commands
// This prevents accidental execution on unsupported systems
func TestInstallZshPluginCompletions_UnsupportedPlatforms(t *testing.T) {
	action := InstallZshPluginCompletions()
...
// TestInstallZshPluginCompletions_ActionConsistency verifies that all platform commands have consistent structure
// This ensures the action is properly configured for safe execution
func TestInstallZshPluginCompletions_ActionConsistency(t *testing.T) {
	action := InstallZshPluginCompletions()
...
// TestInstallZshPluginCompletions_MultipleCalls tests that multiple calls return equivalent actions
// This ensures the factory function is deterministic and safe
func TestInstallZshPluginCompletions_MultipleCalls(t *testing.T) {
	// Test that multiple calls to InstallZshPluginCompletions return equivalent actions
	action1 := InstallZshPluginCompletions()
	action2 := InstallZshPluginCompletions()
...
// TestInstallZshPluginCompletions_ExpectedPlatforms verifies that only expected platforms are configured
// This prevents accidental execution on unexpected platforms
func TestInstallZshPluginCompletions_ExpectedPlatforms(t *testing.T) {
	action := InstallZshPluginCompletions()
...
// TestInstallZshPluginCompletions_CommandStructure tests that the command uses the correct git clone command
// This verifies the command structure without executing it
func TestInstallZshPluginCompletions_CommandStructure(t *testing.T) {
	action := InstallZshPluginCompletions()
...
// TestInstallZshPluginCompletions_CheckCommandStructure tests that the check command properly checks for dependencies
// This ensures the action won't execute if prerequisites are not met
func TestInstallZshPluginCompletions_CheckCommandStructure(t *testing.T) {
	action := InstallZshPluginCompletions()

	// Test that the check command properly checks for git dependency and Oh My Zsh installation
	expectedCheckLinux := "which git && (test -d $HOME/.oh-my-zsh || which omz) && test -d ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-completions"
	expectedCheckMacOS := "which git && (test -d $HOME/.oh-my-zsh || which omz) && test -d ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-completions"
...
// TestInstallZshPluginCompletions_NoCommandExecution ensures that the test never executes actual commands
// This is a safety test to verify that we're only testing configuration, not execution
func TestInstallZshPluginCompletions_NoCommandExecution(t *testing.T) {
	action := InstallZshPluginCompletions()
...
// TestInstallZshPluginCompletions_SafeForCI verifies that the action is safe to use in CI environments
// This test ensures no actual system commands will be executed during testing
func TestInstallZshPluginCompletions_SafeForCI(t *testing.T) {
	action := InstallZshPluginCompletions()
...
// TestInstallZshPluginCompletions_MockExecutorBehavior tests that the action can be safely used with a mock executor
// This demonstrates how to properly mock the action execution without running actual commands
func TestInstallZshPluginCompletions_MockExecutorBehavior(t *testing.T) {
	action := InstallZshPluginCompletions()
	...
	expectedResult := "Zsh completions plugin installation mocked successfully"
